...
class CNN_auto:
	...
	def build_net(self, x, weights, bias):
		...
		# encode_layer_1 = self.maxpool2d(encode_layer_1)

		encode_layer_2 = self.conv2d(encode_layer_1, weights['conv2'], bias['conv2'])
		encode_layer_2 = tf.nn.relu(encode_layer_2)
		# encode_layer_2 = self.maxpool2d(encode_layer_2)

		enocde_layer_output = encode_layer_2

		output_shape_of_dconv1 = tf.pack([tf.shape(x)[0], 28, 28, self.decoder_1])  # output channel: 16
		decode_layer_1 = self.deconv2d(enocde_layer_output, weights['deconv1'], bias['deconv1'], output_shape_of_dconv1)
		decode_layer_1 = tf.nn.relu(decode_layer_1)
		# decode_layer_1 = max_unpool_2x2(decode_layer_1)

		output_shape_of_dconv2 = tf.pack([tf.shape(x)[0], 28, 28, self.decoder_2])  # output channel: 1
		decode_layer_2 = self.deconv2d(decode_layer_1, weights['deconv2'], bias['deconv2'], output_shape_of_dconv2)
		decode_layer_2 = tf.nn.tanh(decode_layer_2)
		# decode_layer_2 = unmaxpool2d(decode_layer_2)
	...
	def unmaxpool2d(self, raveled_argmax, output_shape):
		def unravel_argmax(argmax, shape):
			output_list = []
			output_list.append(argmax // (shape[2] * shape[3]))
		print('raveled_argmax shape{}'.format(raveled_argmax.get_shape()))
